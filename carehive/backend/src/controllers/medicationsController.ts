...
function normalizeTimeOfDay(input: string): string | null {
...
export async function addMedication(req: AuthRequest, res: Response) {
  ...
  const { name, dosage, instructions } = req.body as { name?: string; dosage?: string; instructions?: string };
...
export async function patchMedication(req: AuthRequest, res: Response) {
  const userId = req.userId;
  ...
  const { name, dosage, instructions } = req.body as { name?: string; dosage?: string; instructions?: string };
  const updates: { name?: string; dosage?: string | null; instructions?: string | null } = {};
  if (name !== undefined) updates.name = String(name).trim();
  if (dosage !== undefined) updates.dosage = String(dosage).trim() || null;
  if (instructions !== undefined) updates.instructions = String(instructions).trim() || null;
...
export async function addReminder(req: AuthRequest, res: Response) {
  const userId = req.userId;
  ...
  const { timeOfDay, enabled } = req.body as { timeOfDay?: string; enabled?: boolean };
...
export async function patchReminder(req: AuthRequest, res: Response) {
  const userId = req.userId;
  const medicationId = req.params.medicationId as string;
  ...
  const { timeOfDay, enabled } = req.body as { timeOfDay?: string; enabled?: boolean };
  const updates: { time_of_day?: string; enabled?: boolean } = {};
...
export async function removeReminder(req: AuthRequest, res: Response) {
  const medicationId = req.params.medicationId as string;
  const reminderId = req.params.reminderId as string;
