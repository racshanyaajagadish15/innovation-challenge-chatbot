...
export async function logHealth(req: AuthRequest, res: Response) {
  try {
    ...
    const { medicationTaken, steps, mood, notes, sleepHours, activityMinutes } = req.body as {
 ...
 */
export async function logMedicationIntakes(req: AuthRequest, res: Response) {
  try {
    ...
    const { intakes } = req.body as { intakes?: Array<{ medicationId?: string; taken?: boolean; takenAt?: string }> };
    ...
    const { data, error } = await createMedicationIntakes(rows as any);
    if (error) {
      console.error(error);
 ...
 */
export async function logActivitySession(req: AuthRequest, res: Response) {
  try {
    const userId = req.userId;
    if (!userId) {
      res.status(401).json({ error: 'Unauthorized' });
    ...
    const { type, startTime, endTime, distanceKm, notes } = req.body as {
      type?: string;
      startTime?: string;
      endTime?: string;
      distanceKm?: number;
