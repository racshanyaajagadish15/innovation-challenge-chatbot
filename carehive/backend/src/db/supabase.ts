...
export interface HealthLogRow {
  ...
  notes: string | null;
  timestamp: string;
  source?: string; // 'manual' | 'vision'; may be missing in older DBs
}
...
export async function createHealthLog(row: Omit<HealthLogRow, 'id'>) {
  ...
  const missingSource =
    msg.toLowerCase().includes("health_logs") &&
    msg.toLowerCase().includes('source') &&
    msg.toLowerCase().includes('column');
  if (missingSource) {
    // Remove `source` from the insert payload entirely.
    const { source: _omitSource, ...rowWithoutSource } = row as any;
    const { data: retryData, error: retryError } = await supabase
      .from('health_logs')
      .insert(rowWithoutSource as any)
      .select('*')
      .single();
    if (retryError) return { data: null, error: retryError };
    return { data: retryData as HealthLogRow, error: null };
  }
